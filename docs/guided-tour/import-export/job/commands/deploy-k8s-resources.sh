...
echo "creating dataobject my-first-import"
outputFile="${TMP_DIR}/dataobject-first.yaml"
export namespace="${NAMESPACE}"
inputFile="${COMPONENT_DIR}/installation/dataobject-first.yaml.tpl"
envsubst < ${inputFile} > ${outputFile}
kubectl apply -f ${outputFile}

echo "creating dataobject my-second-import"
outputFile="${TMP_DIR}/dataobject-second.yaml"
export namespace="${NAMESPACE}"
inputFile="${COMPONENT_DIR}/installation/dataobject-second.yaml.tpl"
envsubst < ${inputFile} > ${outputFile}
kubectl apply -f ${outputFile}
