...
var _ = Describe("Components", func() {
	...
	Context("#ResolveInLocalCache", func() {
		It("should resolve a component from a local cache", func() {
			...
			cd, err := components.ResolveInLocalCache(testdatafs, repoCtx, "github.com/openmcp-project/landscaper/legacy-component-cli", "v0.1.0")
			Expect(err).ToNot(HaveOccurred())
			Expect(cd.Name).To(Equal("github.com/openmcp-project/landscaper/legacy-component-cli"))
	...
	Context("#Resolver", func() {
		It("should resolve a component from a local cache", func() {
			...
			cd, err := cdoci.NewResolver(mockOCIClient).
				...
				Resolve(context.TODO(), &repoCtx, "github.com/openmcp-project/landscaper/legacy-component-cli", "v0.1.0")
			Expect(err).ToNot(HaveOccurred())
			Expect(cd.Name).To(Equal("github.com/openmcp-project/landscaper/legacy-component-cli"))
			Expect(cd.Version).To(Equal("v0.1.0"))
		})
		...
		It("should fallback to the oci client if a component is not in the local cache", func() {
					...
					cd := &cdv2.ComponentDescriptor{
						...
						ComponentSpec: cdv2.ComponentSpec{
							ObjectMeta: cdv2.ObjectMeta{
								Name:    "github.com/openmcp-project/landscaper/legacy-component-cli",
								Version: "v0.2.0",
							},
							Provider:           "internal",
			...
			cd, err := cdoci.NewResolver(mockOCIClient).
				WithCache(components.NewLocalComponentCache(testdatafs)).
				Resolve(context.TODO(), &repoCtx, "github.com/openmcp-project/landscaper/legacy-component-cli", "v0.2.0")
			Expect(err).ToNot(HaveOccurred())
			Expect(cd.Name).To(Equal("github.com/openmcp-project/landscaper/legacy-component-cli"))
